...
def _ano_math_kernel(x: torch.Tensor, k: float, b: float, const_term: float) -> torch.Tensor:
    ...
    """
    # f0 logic inlined for fusion
    # term_x = (x - b) / k
    term_x = (x - b) / k
    
    # f0 calculation
    # logsigmoid(2x) is equivalent to log(sigmoid(2x))
    # Note: 45/16 = 2.8125
...
def _compute_ano_loss(
    ...
    """
    Computes the masked ANO loss.
    
    Logic:
    - If Adv >= 0: Loss = -Adv * f(r)
    - If Adv <  0: Loss = -Adv * [2 - f(2-r)]  <-- Symmetric lower bound g(r)
    """
    # 1. 计算两个分支的 f_kernel 值
...
# taken from https://github.com/OpenLMLab/MOSS-RLHF/blob/40b91eb2f2b71b16919addede0341d2bef70825d/ano/ano_trainer.py#L29
...
class ANOTrainer(BaseTrainer):
    """Trainer for Proximal Policy Optimization (ANO).

    For details on ANO, see the paper: [Proximal Policy Optimization
    Algorithms](https://huggingface.co/papers/1707.06347).

    Args:
        args ([`experimental.ano.ANOConfig`]):
            Training arguments.
    ...
    def train(self):
        ...
        # [PRE-COMPUTE CONSTANTS ONCE] - Insert this before the training loop starts
        # Constants for Positive Advantage Case (epsilon = args.cliprange)
        _eps_pos = args.cliprange
        _k_pos = _eps_pos / math.log(2)
        _b_pos = 1 + _eps_pos
        # Calculate ds_d for positive
        _term_d_pos = (1.0 - _b_pos) / _k_pos
        # f0 logic: 45/16 * (0.5 * logsigmoid(2x) - 2 * sigmoid(x))
        _f0_d_pos = (45/16) * (0.5 * math.log(1 / (1 + math.exp(-2 * _term_d_pos))) - 2 * (1 / (1 + math.exp(-_term_d_pos))))
        _ds_d_pos = _k_pos * _f0_d_pos
        _const_term_pos = 1 - _ds_d_pos

        # Constants for Negative Advantage Case (epsilon = -args.cliprange)
        _eps_neg = -args.cliprange
        _k_neg = _eps_neg / math.log(2)
        _b_neg = 1 + _eps_neg
        # Calculate ds_d for negative
        _term_d_neg = (1.0 - _b_neg) / _k_neg
        _f0_d_neg = (45/16) * (0.5 * math.log(1 / (1 + math.exp(-2 * _term_d_neg))) - 2 * (1 / (1 + math.exp(-_term_d_neg))))
        ...
        stats_shape = (args.num_ano_epochs, args.num_mini_batches, args.gradient_accumulation_steps)
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            # Do multiple epochs of ANO training, with a fresh random shuffle in each epoch
            for ano_epoch_idx in range(args.num_ano_epochs):
                ...
                for mini_batch_start in range(0, args.local_batch_size, args.local_mini_batch_size):
                    ...
                    for micro_batch_start in range(0, args.local_mini_batch_size, args.per_device_train_batch_size):
                        with accelerator.accumulate(model):
                            ...
                            # [OPTIMIZED CALL]
                            # Use the JIT compiled function with pre-computed scalars
                            pg_loss_max = _compute_ano_loss(
                                mb_advantage, ratio, 
                                _k_pos, _b_pos, _const_term_pos,
                                _k_neg, _b_neg, _const_term_neg
                            )
                            ...
                            with torch.no_grad():
                                ...
                                approxkl_stats[ano_epoch_idx, minibatch_idx, gradient_accumulation_idx] = approxkl
                                pg_clipfrac_stats[ano_epoch_idx, minibatch_idx, gradient_accumulation_idx] = (
                                    pg_clipfrac
                                )
                                pg_loss_stats[ano_epoch_idx, minibatch_idx, gradient_accumulation_idx] = pg_loss
                                vf_loss_stats[ano_epoch_idx, minibatch_idx, gradient_accumulation_idx] = vf_loss
                                vf_clipfrac_stats[ano_epoch_idx, minibatch_idx, gradient_accumulation_idx] = (
                                    vf_clipfrac
                                )
                                entropy_stats[ano_epoch_idx, minibatch_idx, gradient_accumulation_idx] = entropy.mean()
                                ratio_stats[ano_epoch_idx, minibatch_idx, gradient_accumulation_idx] = ratio.mean()
