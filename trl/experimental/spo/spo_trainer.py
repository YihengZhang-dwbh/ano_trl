...
# taken from https://github.com/OpenLMLab/MOSS-RLHF/blob/40b91eb2f2b71b16919addede0341d2bef70825d/spo/spo_trainer.py#L29
...
class SPOTrainer(BaseTrainer):
    """Trainer for Proximal Policy Optimization (SPO).

    For details on SPO, see the paper: [Proximal Policy Optimization
    Algorithms](https://huggingface.co/papers/1707.06347).

    Args:
        args ([`experimental.spo.SPOConfig`]):
            Training arguments.
    ...
    _paper = {
        ...
        "citation": textwrap.dedent("""\
            @article{mziegler2019fine-tuning,
                title        = {{Fine-Tuning Language Models from Human Preferences}},
                author       = {Daniel M. Ziegler and Nisan Stiennon and Jeffrey Wu and Tom B. Brown and Alec Radford and Dario Amodei and Paul F. Christispo and Geoffrey Irving},
                year         = 2019,
    ...
    def train(self):
        ...
        stats_shape = (args.num_spo_epochs, args.num_mini_batches, args.gradient_accumulation_steps)
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            # Do multiple epochs of SPO training, with a fresh random shuffle in each epoch
            for spo_epoch_idx in range(args.num_spo_epochs):
                    ...
                    for micro_batch_start in range(0, args.local_mini_batch_size, args.per_device_train_batch_size):
                        with accelerator.accumulate(model):
                            ...
                            with torch.no_grad():
                                ...
                                approxkl_stats[spo_epoch_idx, minibatch_idx, gradient_accumulation_idx] = approxkl
                                pg_clipfrac_stats[spo_epoch_idx, minibatch_idx, gradient_accumulation_idx] = (
                                    pg_clipfrac
                                )
                                pg_loss_stats[spo_epoch_idx, minibatch_idx, gradient_accumulation_idx] = pg_loss
                                vf_loss_stats[spo_epoch_idx, minibatch_idx, gradient_accumulation_idx] = vf_loss
                                vf_clipfrac_stats[spo_epoch_idx, minibatch_idx, gradient_accumulation_idx] = (
                                    vf_clipfrac
                                )
                                entropy_stats[spo_epoch_idx, minibatch_idx, gradient_accumulation_idx] = entropy.mean()
                                ratio_stats[spo_epoch_idx, minibatch_idx, gradient_accumulation_idx] = ratio.mean()
